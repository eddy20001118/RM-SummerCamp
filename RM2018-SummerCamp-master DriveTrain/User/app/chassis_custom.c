...
{
  static float rotate_ratio_f = ((WHEELBASE+WHEELTRACK)/2.0f - GIMBAL_OFFSET)/RADIAN_COEF; //转换为角度
...
void chassis_twist_handle(float direction,float currentDirection)
{
  /* 扭腰周期时间 */
  ...
  chassis.vw = pid_calc(&pid_chassis_angle, currentDirection, direction);
}
